...
if Config.Framework == "qb" then
    QBCore.Functions.CreateUseableItem("hw_packsurprise", function(source, item) 
        ...
        if Config.Cooldown.Enable then if not canOpen then return TriggerClientEvent('m-HotWheels:Client:Notify', src, "Şu anda açamazsınız.", "error", 5000) end end
...
elseif Config.Framework == "esx" then
    ...
    ESX.RegisterUsableItem('hw_packsurprise', function(source)
        local source = source
        local xPlayer  = ESX.GetPlayerFromId(source)
    
        if not xPlayer then return end
        if Config.Cooldown.Enable then if not canOpen then return TriggerClientEvent('m-HotWheels:Client:Notify', source, "Şu anda açamazsınız.", "error", 5000) end end
        if xPlayer.removeInventoryItem('hw_packsurprise', 1)
        then
            TriggerClientEvent('m-HotWheels:Client:OpenPackSurprise', source)
            if Config.Cooldown.Enable then Cooldown() end
        end
    ...
    ESX.RegisterUsableItem('hw_boxsurprise', function(source)
        local source = source
        local xPlayer  = ESX.GetPlayerFromId(source)
    
        if not xPlayer then return end
        if Config.Cooldown.Enable then if not canOpen then return TriggerClientEvent('m-HotWheels:Client:Notify', source, "Şu anda açamazsınız.", "error", 5000) end end
        if xPlayer.removeInventoryItem('hw_boxsurprise', 1)
        then
            TriggerClientEvent('m-HotWheels:Client:OpenBoxSurprise', source)
            if Config.Cooldown.Enable then Cooldown() end
        end
    ...
    ESX.RegisterUsableItem('hw_boxsurprise_2f2f', function(source)
        local source = source
        local xPlayer  = ESX.GetPlayerFromId(source)
    
        if not xPlayer then return end
        if Config.Cooldown.Enable then if not canOpen then return TriggerClientEvent('m-HotWheels:Client:Notify', source, "Şu anda açamazsınız.", "error", 5000) end end
        if xPlayer.removeInventoryItem('hw_boxsurprise_2f2f', 1)
        then
            TriggerClientEvent('m-HotWheels:Client:Open2FastSurprise', source)
            if Config.Cooldown.Enable then Cooldown() end
        end
    ...
    ESX.RegisterUsableItem('hw_box01', function(source,item)
        ...
        if not xPlayer then return end
        TriggerClientEvent("m-HotWheels:Client:OpenWhiteBox", source)
    end)
    ...
    ESX.RegisterUsableItem('hw_box02', function(source,item)
        ...
        if not xPlayer then return end
        TriggerClientEvent("m-HotWheels:Client:OpenGreyBox", source)
    end)
    ...
    ESX.RegisterUsableItem('hw_box03', function(source,item)
        ...
        if not xPlayer then return end
        TriggerClientEvent("m-HotWheels:Client:OpenBlueBox", source)
    end)
...
AddEventHandler("m-HotWheels:Server:OpenPackSurprise", function()
    ...
    elseif Config.Framework == "esx" then
        ...
        if chance <= Config.HotWheels.Settings.RareCars and chance > Config.HotWheels.Settings.UltraRareCars then
            ...
            xPlayer.addInventoryItem(rareDrop, 1)
           -- TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items[rareDrop], 'add', 1)


        elseif chance <= Config.HotWheels.Settings.UltraRareCars then

            xPlayer.addInventoryItem(ultrarareDrop, 1)
          -- TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items[ultrarareDrop], 'add', 1)


        else
            xPlayer.addInventoryItem(normalDrop, 1)
            --TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items[normalDrop], 'add', 1)
        end
    end
